...
| GPU voice turn | **refuted — do not cite.** The 719.82 ms response latency in that report comes from a `speech_end_to_transcript_ms=0.0` stub for a segment with no endpointer, so it omits the ~3.8 s of LLM decode its own `llm_total_ms` records. Fixed in `593e97b` (the field is now `None`); the sweep has not been re-run since. | superseded by `results/live/turns.jsonl` (12 turns, 4435.5 ms p50) |
